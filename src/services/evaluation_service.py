...
def _artifact_path_xgb(config: AppConfig, scope: str, key_vals) -> Path:
...
def evaluate_xgboost_pipeline(session, config: AppConfig, scope: str) -> Dict[str, Any]:
    try:
        df = _fetch_sales_aggregated_any(session, scope=scope)
        if df.empty:
            return {"status": "no_data", "message": "No sales data available from DB", "scope": scope}
        ...
        results = []
        for key_vals, grp in ts.groupby(keys):
            path = _artifact_path_xgb(config, scope, key_vals)
            if not path.exists():
                continue
            try:
        ...
        if not results:
            return {"status": "no_models", "message": "No xgboost models or evaluable groups found", "scope": scope}
        ...
        return {"status": "error", "message": "evaluate_xgboost failed", "error": str(e)}


def compare_models_pipeline(session, config: AppConfig, scope: str) -> Dict[str, Any]:
    out: Dict[str, Any] = {"status": "ok", "scope": scope, "models": {}}
    prop = evaluate_prophet_pipeline(session, config, scope)
    lgb = evaluate_lightgbm_pipeline(session, config, scope)
    xgb = evaluate_xgboost_pipeline(session, config, scope)
    out["models"]["prophet"] = prop
    out["models"]["lightgbm"] = lgb
    out["models"]["xgboost"] = xgb
    def pick(m, k):
        try:
            return m.get("summary", {}).get(k)
        except Exception:
            return None
    out["summary"] = {
        "prophet": {"rmse": pick(prop, "rmse"), "mae": pick(prop, "mae"), "mape": pick(prop, "mape")},
        "lightgbm": {"rmse": pick(lgb, "rmse"), "mae": pick(lgb, "mae"), "mape": pick(lgb, "mape")},
        "xgboost": {"rmse": pick(xgb, "rmse"), "mae": pick(xgb, "mae"), "mape": pick(xgb, "mape")},
    }
    return out
